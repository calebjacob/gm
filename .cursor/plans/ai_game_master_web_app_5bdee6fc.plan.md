...
- **Implementation**: A single migration runner (e.g. `server/db/migrate.ts` or `migrations.ts`) that: opens the DB, runs `PRAGMA user_version`, runs pending migration steps in a transaction, then sets `PRAGMA user_version` to the latest version. Invoke this runner **on app start** before any other DB access.
...
| DB         | `server/db/index.ts` (connection, load extensions, **run migrations on app start**). `server/db/migrate.ts` (or `migrations.ts`): migration runner that reads `PRAGMA user_version`, applies pending migrations in a transaction (using ALTER TABLE for add/rename; create-new-table + migrate + drop + rename for other changes), then sets `user_version`. Optional: `server/db/schema.sql` or per-version SQL files for reference. Load sqlite-vec and sqlite-rembed before migrations that create vec0 tables.                                                                                                  |
