...
export default function Rulesets() {
  const { rulesets } = useLoaderData() as Route.ComponentProps["loaderData"];
  return (
    <Box sx={{ p: 3, maxWidth: 900, mx: "auto" }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
        <Typography variant="h4">Rulesets</Typography>
        <Button component={Link} to="/rulesets/new" variant="contained" startIcon={<Plus size={18} />}>
          New Ruleset
        </Button>
      </Box>
      <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
        {rulesets.length === 0 ? (
          <Typography color="text.secondary">No rulesets yet. Create one from PDF/MD/TXT files.</Typography>
        ) : (
          rulesets.map((r) => (
            <Card key={r.id} component={Link} to={`/rulesets/${r.id}`} sx={{ textDecoration: "none", color: "inherit" }}>
              <CardContent>
