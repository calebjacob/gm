	...
	.handler(async function* ({ data }) {
		try {
			...
			const module: ModuleSchema = {
				...
				name,
				description:
					"Some description about the contents of the module. Generated by the AI.",
			...
			for (const textFileChunk of textFile.chunks) {
				...
				yield {
					type: "chunk-start" as const,
					chunk: textFileChunk,
				};

				const { embedding: chunkEmbedding } = await embedContentServer(
					textFileChunk.content,
				);

				yield {
					type: "chunk-embedded" as const,
					chunkEmbedding,
				};
				...
				yield {
					type: "chunk-inserted" as const,
					chunk,
				};

				const { data: categorizedChunks } =
					await generateStructuredContentServer({
				...
				for (const categorizedChunk of categorizedChunks) {
					...
					yield {
						type: "categorized-chunk-embedded" as const,
					...
					yield {
						type: "categorized-chunk-inserted" as const,
						categorizedContentChunk,
					};
				}

				yield {
					type: "chunk-end" as const,
					module,
					chunk,
					categorizedChunks,
				};
...
export function ModuleSelector({
	...
	const selectedModules = useMemo(() => {
		return selectedModuleIds
			.map((id) =>
				getModulesQuery.data?.modules.find((module) => module.id === id),
			)
			.filter((module) => !!module);
	}, [getModulesQuery.data?.modules, selectedModuleIds]);
		...
		for (const file of files) {
			...
			try {
				for await (const message of await createModule({
					...
					} else if (message.type === "chunk-end") {
						setProcessingModules((prev) =>
							prev.map((m) =>
								m.file === file
									? {
				...
				<Card key={module.id} padding={1} gap={1}>
					<Row align="center" gap={1}>
						...
						<Stack gap={0.5}>
							...
							{module.description && <Text>{module.description}</Text>}
						</Stack>
					</Row>
				</Card>
			))}
